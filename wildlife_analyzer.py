...
class CONFIG:
    ...
    MIN_CONTENT_THRESHOLD = 0.005 # Fraction of pixels that must be edges (0.5%)
    ...
    def load(cls):
        ...
        try:
            ...
            import importlib
            importlib.reload(user_config) # Reload in case of changes during runtime
            
            cls.VIDEO_PATH = getattr(user_config, "VIDEO_PATH", cls.VIDEO_PATH)
            cls.SNAPSHOT_INTERVAL = getattr(user_config, "SNAPSHOT_INTERVAL", cls.SNAPSHOT_INTERVAL)
            cls.BLUR_THRESHOLD = getattr(user_config, "BLUR_THRESHOLD", cls.BLUR_THRESHOLD)
            ...
            cls.LOCATION_CONTEXT = getattr(user_config, "LOCATION_CONTEXT", cls.LOCATION_CONTEXT)
            cls.CONFIDENCE_THRESHOLD = getattr(user_config, "CONFIDENCE_THRESHOLD", cls.CONFIDENCE_THRESHOLD)
            cls.MIN_CONTENT_THRESHOLD = getattr(user_config, "MIN_CONTENT_THRESHOLD", cls.MIN_CONTENT_THRESHOLD)
            cls.MOTION_THRESHOLD = getattr(user_config, "MOTION_THRESHOLD", cls.MOTION_THRESHOLD)
            ...
            cls.RETRY_CONFIDENCE_THRESHOLD = getattr(user_config, "RETRY_CONFIDENCE_THRESHOLD", cls.RETRY_CONFIDENCE_THRESHOLD)
            cls.ANALYSIS_PROMPT = getattr(user_config, "ANALYSIS_PROMPT", cls.ANALYSIS_PROMPT)
...
def detect_motion(frame1, frame2):
    ...
    p0 = cv2.goodFeaturesToTrack(gray1, maxCorners=200, qualityLevel=0.01, minDistance=30, blockSize=3)
    ...
    # estimateAffinePartial2D is more robust for simple camera shake than findHomography
...
def ensure_directories(clear=False):
    ...
    """
    for directory in [CONFIG.FRAMES_DIR, CONFIG.REJECTED_DIR, CONFIG.BLURRY_DIR, CONFIG.MANUAL_DIR]:
...
def extract_frames():
    ...
    """
    print(f"--- Starting Extraction Mode ---")
    ...
    while True:
        ...
        if CONFIG.SMART_EXTRACT:
            # Hybrid Smart Mode:
            # 1. Peek at the first frame to see if it's worth scanning the whole second
            ...
            if not has_cont and not is_moving:
                ...
                # Assume the whole second is likely empty. Skip the expensive scan.
                final_frame = first_frame
                # We still need a blur score for the logic below (even if ignored for rejection)
                ...
                # Content OR Motion detected! Worth scanning for the sharpest frame.
        ...
        if final_frame is not None:
             # Re-evaluate with thresholds (since is_blurry logic is just variance calc)
             is_blur = best_blur_score < CONFIG.BLUR_THRESHOLD
             has_cont, content_score = has_content(final_frame)
             
             # Re-check motion for the processing decision (Fast Mode needs this)
             is_moving = False
             ...
             if not CONFIG.SMART_EXTRACT:
                 ...
                 if ret_next:
                     is_moving, motion_score = detect_motion(final_frame, next_frame_fast)
    ...
    logging.info(f"Saved: {saved_count} sharp, interesting frames to '{CONFIG.FRAMES_DIR}'.")
    logging.info(f"Rejected content: {rejected_empty_count} frames to '{CONFIG.REJECTED_DIR}'.")
    logging.info(f"Rejected blurry: {blurry_count} frames to '{CONFIG.BLURRY_DIR}'.")
    logging.info(f"Estimated Analysis Cost: ${estimated_cost:.4f} (based on Gemini Flash rates).")
    logging.info(f"Please review rejected folders and move any missed fish to '{CONFIG.FRAMES_DIR}' before analysis.")
...
@retry(wait=wait_exponential(multiplier=2, min=4, max=120), stop=stop_after_attempt(10))
...
def get_video_creation_date(video_path):
    ...
    """
    try:
        # Command: ffprobe -v quiet -select_streams v:0 -show_entries stream_tags=creation_time -of default=noprint_wrappers=1:nokey=1 [FILE]
        cmd = [
            "ffprobe", 
            "-v", "quiet", 
            "-select_streams", "v:0", 
        ...
        print(f"Warning: Could not extract video date: {e}")
...
def sort_session_log(log_path):
    ...
    try:
        ...
        print(f"Log file sorted: {os.path.basename(log_path)}")
    except Exception as e:
        print(f"Warning: Could not sort log file: {e}")

def analyze_frames():
    """
    Mode B: Send images in FRAMES_DIR and MANUAL_DIR to Gemini Flash.
    """
    print(f"--- Starting Analysis Mode ---")
    ...
    if not image_files:
        print(f"No images found in {CONFIG.FRAMES_DIR} or {CONFIG.MANUAL_DIR}. Run 'extract' mode first.")
        return
    ...
    if not os.path.exists(log_file):
        with open(log_file, "w", newline='', encoding='utf-8') as f:
            ...
            writer.writerow(["timestamp","file","common_name","scientific_name","confidence"])
    else:
        ...
        try:
            with open(log_file, "r", newline='', encoding='utf-8') as f:
                ...
                for row in reader:
                     # Check if file column exists (it might be index 1 based on old logic, but DictReader uses header)
                     if row.get("file"):
                         processed_files.add(row["file"])
            print(f"Resuming analysis. {len(processed_files)} images already processed.")
        except Exception as e:
            print(f"Error reading existing log: {e}")

    print(f"Found {len(image_files)} images total ({len(files_manual)} manual overrides).")
    ...
    for img_path in tqdm(image_files, desc="Analyzing"):
        ...
        try:
            ...
            # Note: We use safe formatting or just .format() if we trust the string
            try:
                # Handle potential missing placeholders in user config safely
                scan_prompt = CONFIG.ANALYSIS_PROMPT.format(location=CONFIG.LOCATION_CONTEXT, date=date_context)
            except Exception as e:
            ...
            if not common_name or common_name.lower() in ["none", "null", "unknown", "rock", "sand", "gravel", "water", "coral rubble"]:
                logging.info(f"  > Skipping inanimate/empty frame ({common_name}).")
                continue
            ...
            if CONFIG.SMART_RETRY and confidence < CONFIG.RETRY_CONFIDENCE_THRESHOLD:
                logging.info(f"  > Low confidence ({confidence:.2f} < {CONFIG.RETRY_CONFIDENCE_THRESHOLD}). Attempting Smart Retry...")
                logging.debug(f"Searching for better frame at {timestamp_str}...")
                
                # 1. Find better frame
                better_frame, b_score = find_best_frame_at_timestamp(CONFIG.VIDEO_PATH, timestamp_str)
                
                if better_frame is not None:
                     logging.info(f"  > Found replacement frame (Blur Score: {b_score:.1f}). Re-analyzing...")
                     ...
                     # 4. Call API again
                     try:
                         response_retry = call_gemini_api(client, scan_prompt, pil_image)
                         text_retry = response_retry.text.strip()
                         
                         if "```" in text_retry:
                             parts = text_retry.split("```")
                             for part in parts:
                                 if "{" in part:
                                     text_retry = part.replace("json", "").strip()
                                     break
                         
                         data_retry = {}
                         try:
                            data_retry = json.loads(text_retry)
                         except:
                            try:
                                data_retry = ast.literal_eval(text_retry)
                            except:
                                pass
                         
                         if isinstance(data_retry, list) and len(data_retry) > 0:
                             data_retry = data_retry[0]
                             
                         new_conf = float(data_retry.get("confidence", 0.0))
                         new_name = data_retry.get("common_name")
                         
                         # Check if retry thinks it's inanimate too
                         if not new_name or new_name.lower() in ["none", "null", "unknown", "rock", "sand", "gravel", "water", "coral rubble"]:
                              logging.info(f"  > Retry found inanimate object ({new_name}). Skipping.")
                              continue
                         
                         if new_conf > confidence:
                             logging.info(f"  > Retry SUCCESS! Confidence improved: {confidence:.2f} -> {new_conf:.2f}")
                             # Accept new data
                             common_name = new_name
                             scientific_name = data_retry.get("scientific_name") or "Unknown"
                             confidence = new_conf
                         else:
                             logging.info(f"  > Retry failed to improve confidence ({new_conf:.2f}). Keeping original.")
                             
                     except Exception as retry_err:
                         logging.warning(f"  > Retry API call failed: {retry_err}")
                else:
            ...
            with open(log_file, "a", newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow([timestamp_str, base_name, common_name, scientific_name, f"{confidence:.2f}"])
            ...
            if confidence >= CONFIG.CONFIDENCE_THRESHOLD:
                # Chapter format: "00:05 - Sergeant Major (Abudefduf saxatilis) [0.95]"
                entry_name = f"{common_name} ({scientific_name})"
                if entry_name != last_species_name:
                    youtube_chapters.append(f"{timestamp_str} - {entry_name} [{confidence:.2f}]")
            ...
            time.sleep(10) # Also sleep on error!
...
def tune():
    ...
    logging.info("This script analyzes your manual sorting to find the best thresholds.")
    ...
    if not good_stats:
        logging.error(f"Error: No images found in '{dir_good}'. You must have at least one 'good' frame to tune thresholds.")
        return
    ...
    false_positive_rejected = [s for s in rejected_stats if s['content'] > new_content_thresh]
    
    print("\n--- Analysis Results ---")
    print(f"Total Good Files: {len(good_stats)}")
    print(f"  Min Blur Score found: {min_good_blur:.2f}")
    print(f"  Min Content Score found: {min_good_content:.5f}")
    
    print(f"\nProposed Config:")
    print(f"  BLUR_THRESHOLD:        {new_blur_thresh:.4f} (was {CONFIG.BLUR_THRESHOLD})")
    print(f"  MIN_CONTENT_THRESHOLD: {new_content_thresh:.5f} (was {CONFIG.MIN_CONTENT_THRESHOLD})")
    
    print("\n--- Impact Analysis ---")
    if false_positive_blurry:
        logging.warning(f"WARNING: {len(false_positive_blurry)} images from '{dir_blurry}' will now be considered SHARP.")
        logging.warning(f"  (They have blur scores >= {new_blur_thresh:.4f}).")
    else:
        logging.info(f"Perfect Separation! No images from '{dir_blurry}' will be falsely accepted.")

    if false_positive_rejected:
        logging.warning(f"WARNING: {len(false_positive_rejected)} images from '{dir_rejected}' will now be considered VALID CONTENT.")
        logging.warning(f"  (They have content scores > {new_content_thresh:.5f}).")
    else:
        logging.info(f"Perfect Separation! No images from '{dir_rejected}' will be falsely accepted.")
    ...
    if confirm.lower() == 'y':
        ...
        try:
            ...
            for line in lines:
                ...
                # Update MIN_CONTENT_THRESHOLD
                    ...
                    if "#" in line:
                        comment = " # " + line.split("#", 1)[1].strip()
                    new_lines.append(f"MIN_CONTENT_THRESHOLD = {new_content_thresh}{comment}\n")
                else:
            ...
            if choice == 's':
                ...
                print("WARNING: This will clear your manual sorting folders and re-extract from the video.")
                extract_frames()
            else:
        ...
        print("Cancelled. Config not updated.")
...
def refine_selected_frames():
    ...
    if not candidates:
        ...
        logging.info(f"Checked {CONFIG.MANUAL_DIR} (All) and {CONFIG.FRAMES_DIR} (Only files with _blur/_empty suffixes).")
        return

    print(f"Found {len(candidates)} images to refine.")
...
def archive_log():
    ...
    try:
        with open(current_log, "r", newline='', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                # Expects keys: timestamp,file,common_name,scientific_name,confidence
    ...
    # Check Master Log
    existing_keys = set()
    fieldnames = ["date", "location", "video_name", "timestamp", "common_name", "scientific_name", "confidence"]
    ...
    if file_exists:
        try:
            ...
            print(f"Warning: Could not read master log (might be corrupt): {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("mode", choices=["extract", "analyze", "refine", "tune", "archive"], help="Operation mode")
    ...
    if len(sys.argv) == 1:
        ...
        print("              - AUTO-REFINES manual frames (better quality) before tuning.\n")
